        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!--                以前的路由是A路由跳转到B路由，这里是加入购物车，进行路由跳转之前，发请求把你的
                                      购买产品信息通过请求的方式通知服务器，服务器进行相应的存储-->
...
export default {
  ...
  mounted() {
    ...
    this.$store.dispatch('getGoodInfo', skuId)
  },
  computed: {
    ...mapGetters(['categoryView', 'skuInfo', 'spuSaleAttrList']),
    //  父组件传递给子组件数据  给子组件的数据留一个空数组兜底 以防假报错
    skuImageList() {
      //如果服务器数据没有返回，skuInfo是一个空对象
  ...
  methods: {
    ...
    async addShopcar() {
      ...
      try {
        ...
        //  路由跳转
        //  在路由跳转的时候还需要将产品的信息带给下一个级的路由组件
        //   this.$router.push({name:'addcartsuccess',query:{skuInfo:this.skuInfo,skuNum:this.skuNum}})  //不建议使用这种方式传参，这种会使路由地址比较混乱
        // 产品信息的数据【比较复杂：skuinfo】,通过本地存储，(不会持久化，会话结束数据在消失)  skuNum:购买数量
        //本地存储和会话存储，一般都是存储字符串
